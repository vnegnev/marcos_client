...
def _resolve_hardware_config(hardware_config):
...
def col2buf(col_idx, value, hardware_config=None):
    ...
    grad_board = hardware_config.grad_board
...
def csv2bin(path, quick_start=False, initial_bufs=np.zeros(MARGA_BUFS, dtype=np.uint16), latencies = np.zeros(MARGA_BUFS, dtype=np.int32), hardware_config=None):
    ...
    for k, dd in enumerate(data_diff):
        ...
        for col_idx, value in zip(dw + 1, data[k + 1][dw + 1]):
            buf_idces, vals, masks = col2buf(col_idx, value, hardware_config=hardware_config)
    ...
    return cl2bin(changelist, changelist_grad, initial_bufs, hardware_config=hardware_config)

def dict2bin(sd, initial_bufs=np.zeros(MARGA_BUFS, dtype=np.uint16), latencies = np.zeros(MARGA_BUFS, dtype=np.int32), hardware_config=None):
    ...
    for k, vals in sd.items(): # iterate over dictionary keys
        ...
        buf_idces, values, masks = col2buf(col_idx, vals[1], hardware_config=hardware_config) # single element or array of values
...
def cl2bin(changelist, changelist_grad,
    ...
    for event in changes:
        ...
        for m, (ind, dat) in enumerate(zip(event[1], event[2])):
            ...
            #buf_empty = btli <= m
            buf_empty = btli <= m # or <= m, need to check against m since with successive cycles, remaining buffers will empty out
